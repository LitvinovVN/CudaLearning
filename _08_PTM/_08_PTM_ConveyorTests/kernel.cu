...
//#define GridNx 1000 // Размерность расчетной сетки по оси x
//#define GridNy 1000 // Размерность расчетной сетки по оси y
//#define GridNz 1000 // Размерность расчетной сетки по оси z
...
#define ThreadsNumber 9 // от 1 до CudaCoresNumber
...
#define BlockSizeY 10 /*SharedMemorySize/BlockSizeX*/ // Размерность блока по Y от 1 до CudaCoresNumber
...
#define GridNz 1 // Размерность расчетной сетки по оси z
#define GridN GridNx*GridNy*GridNz // Суммарное число узлов расчетной сетки
#define GridXY GridNx * GridNy // Число узлов в плоскости XY, т.е. в одном слое по Z
...
__global__ void ptmKernel1(double* r, double* c0, double* c2, double* c4, double* c6, unsigned int size, double omega)
{
    ...
    //printf("202: idx = %d\n", idx);
    int currentY = 1; // 0 - граница, берём 1
